    ...
    try {
      ...
      while (true) {
        ...
        const { data: existing } = await supabase
          .from('rooms').select('id, game_status').eq('room_code', code);
      ...
      setLoading(false);
      setError(t.createFailed + e.message);
    }
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      const { data: rooms, error } = await supabase
        .from('rooms').select('*').eq('room_code', roomCode.trim()).limit(1);
      ...
      setLoading(false);
      setError(t.joinFailed + e.message);
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        {!mode && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            <Button onClick={() => setMode('create')} disabled={!username.trim()}
              className="w-full h-14 text-base font-bold rounded-xl">
              <UserPlus className="w-5 h-5 mr-2" />{t.createRoom}
            </Button>
            <Button onClick={() => setMode('join')} disabled={!username.trim()}
